class Mailbox
  ...
  #Returns the conversations in the custom mailbox of messageable
  #
  #Same as conversations({:mailbox_name => 'somecustomvalue'})
  def custom(options={})
    if options[:mailbox_name].present?
      options = options.merge(mailbox_name: options[:mailbox_name])
    else
      options = options
    end
