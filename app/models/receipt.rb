class Receipt < ActiveRecord::Base
  ...
  belongs_to :receiver, :polymorphic => :true, with_deleted: true
  ...
  scope :custom, lambda { |mailbox_name| where(:mailbox_type => mailbox_name) }
  ...
  class << self
    ...
    #Marks all the receipts from the realtion to custom mailbox
    def move_to_mailbox(options={})
      if options[:mailbox_name].present?
        update_receipts({:mailbox_type => options[:mailbox_name]})
      end
    end
